...
(defn with-schema [f]
  ...
  (spit "db.edn" {:dev context})
  (let [conn (db/connect context)]
...
(deftest insert-test
  (testing "insert with nested map"
    (is (= {:name "name" :email "email@email.com" :password "password"
            :id 1 :updated-at nil}
           (dissoc (db/insert @connection {:account {:name "name" :email "email@email.com" :password "password"}})
             :created-at)))
